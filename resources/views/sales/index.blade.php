<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center px-6 py-6 bg-white shadow-sm rounded-lg">
            ...
            <div
                class="flex items-center space-x-2 bg-gradient-to-r from-emerald-50 to-white px-8 py-4 rounded-lg border border-emerald-100">
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-xl">
                <div class="p-6">
                    <div class="mb-4 flex space-x-4">
                        <input type="date" id="fromDate" class="px-4 py-2 border rounded-lg"
                            onchange="filterSales()">
                        <input type="date" id="toDate" class="px-4 py-2 border rounded-lg"
                            onchange="filterSales()">
                        <label for="customerSelect" class="block text-sm font-medium text-gray-700">Select
                            Customer</label>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody id="salesTable" class="bg-white divide-y divide-gray-200">
                                @forelse ($sales->sortByDesc('created_at') as $sale)
                                    @php
                                        $totalValue = $sale->products->sum(
                                            fn($product) => $product->pivot->quantity * $product->selling_price,
                                        );
                                    @endphp
                                    <tr class="hover:bg-blue-50 transition-colors duration-200">
                                        ...
                                        @php
                                            $isDeleted = $sale->customer && $sale->customer->trashed();
                                        @endphp

                                        <td
                                            class="px-6 py-4 text-sm font-medium {{ $isDeleted ? 'text-red-600' : 'text-gray-900' }}">
                                            ...
                                            <div class="space-y-2">
                                                @foreach ($sale->products as $product)
                                                    <div class="text-sm"><span
                                                            class="text-gray-900 font-medium">{{ $product->name }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900 total-value">Rs.
                                            {{ number_format($totalValue, 2) }}</td>
                                        <td class="px-6 py-4 text-sm font-medium text-emerald-600">
                                            Rs.
                                            {{ number_format($sale->products->sum(fn($product) => $product->pivot->quantity * ($product->selling_price - $product->original_price)), 2) }}
                                        </td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $sale->created_at }}
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">No sales found.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                            <tfoot>
                                <tr class="bg-gray-100">
                                    ...
                                    <td id="totalSalesSum" class="px-6 py-4 text-left font-bold text-emerald-600">Rs.
                                        0.00</td>
                                    <td colspan="2"></td>
                                </tr>
                            </tfoot>
    ...
    <script>
        ...
        function filterSales() {
            ...
            rows.forEach(row => {
                ...
                const totalValue = parseFloat(row.cells[5]?.textContent.replace('Rs.', '').replace(',', '')
                .trim()) || 0;
        ...
        function downloadExcel() {
            ...
            const data = [
                ['Customer', 'Product', 'Qty', 'Total Value', 'Profit', 'Date']
            ];

            rows.forEach(row => {
                const customer = row.cells[1]?.textContent.trim() || "";
                const products = Array.from(row.cells[2]?.querySelectorAll('div') || []).map(div => div.textContent
                    .trim()).join(', ');
                const quantities = Array.from(row.cells[3]?.querySelectorAll('div') || []).map(div => div
